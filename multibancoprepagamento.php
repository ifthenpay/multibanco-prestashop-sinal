...
class MultiBancoPrePagamento extends PaymentModule
{
	private $_html = '';
	...
	public $ifmb_os_0;
	public $ifmb_os_1;
	public $ifmb_percentagem;
	public $ifmb_valorminimo;

	public function __construct()
	{
		$this->name = 'multibancoprepagamento';
		...
		$config = Configuration::getMultiple(array('MULTIBANCO_ENTIDADE', 'MULTIBANCO_SUBENTIDADE', 'MULTIBANCO_CHAVE_ANTI_PHISHING', 'MULTIBANCO_OS_0', 'MULTIBANCO_OS_1', 'MULTIBANCO_PERCENTAGEM', 'MULTIBANCO_VALOR_MINIMO'));
		...
		if (isset($config['MULTIBANCO_PERCENTAGEM']))
			$this->ifmb_percentagem = $config['MULTIBANCO_PERCENTAGEM'];
			
		if (isset($config['MULTIBANCO_VALOR_MINIMO']))
			$this->ifmb_valorminimo = $config['MULTIBANCO_VALOR_MINIMO'];			

		parent::__construct();

		$this->displayName = 'Pagamento por contra reembolso';
		$this->description = 'Permitir &agrave;s empresas, lojas on-line, escolas, clubes, associa&ccedil;&otilde;es e munic&iacute;pios a emiss&atilde;o de Refer&ecirc;ncias Multibanco nos seus documentos ou site, para pré-pagamento por serviços ou bens, que podem ser pagas na rede Multibanco ou Home Banking.';
    ...
    public function uninstall()
	{
		...
		if (!Configuration::deleteByName('MULTIBANCO_ENTIDADE') OR
				...
				!Configuration::deleteByName('MULTIBANCO_PERCENTAGEM') OR
				!Configuration::deleteByName('MULTIBANCO_VALOR_MINIMO') OR
				!parent::uninstall())
			return false;
	...
	function hookdisplayAdminOrder($params)
	{
		...
		//verifica se o m�todo da encomenda � mesmo este ou n�o...
		if($order->payment != $this->displayName)
			return;

		$tot = $this->getPrePaidValue($order->total_paid);

		$ref = $this->GenerateMbRef($this->ifmb_entidade,$this->ifmb_subentidade,$order_id,$tot);
	...
	function hookdisplayOrderDetail($params)
	{
		...
		$total = $this->getPrePaidValue($params["order"]->total_paid);
		
		$ref = $this->GenerateMbRef($this->ifmb_entidade,$this->ifmb_subentidade,$order_id,$total);
	...
	private function _postProcess()
	{
		if (Tools::isSubmit('btnSubmit'))
		{
			...
			Configuration::updateValue('MULTIBANCO_PERCENTAGEM', Tools::getValue('percentagem'));
			Configuration::updateValue('MULTIBANCO_VALOR_MINIMO', Tools::getValue('valor_minimo'));
		}
	...
	private function _displayMultibanco()
	{
		...
		<table style="margin-bottom: 20px;">
			<tr>
				<td>
					<img src="../modules/multibancoprepagamento/logo_mb.png" style="margin-right:15px;width: 77px;">
				</td>
				<td>
	...
	private function _displayForm()
	{
		...
		'<form action="'.Tools::htmlentitiesUTF8($_SERVER['REQUEST_URI']).'" method="post">
			<fieldset>
			<legend><img src="../modules/multibancoprepagamento/logo.gif" />Dados do Contracto</legend>
			...
			<fieldset>
				<legend><img src="../modules/multibancoprepagamento/logo.gif" />Forma de pagamento</legend>
				<table border="0" width="500" cellpadding="0" cellspacing="0" id="form">
					<tr><td width="130" style="height: 35px;">'.$this->l('Percentagem').'</td><td><input type="text" name="percentagem" value="'.htmlentities(Tools::getValue('percentagem', $this->ifmb_percentagem), ENT_COMPAT, 'UTF-8').'" style="width: 300px;" /></td></tr><tr><td width="130" style="height: 35px;">'.$this->l('Valor mínimo').'</td><td><input type="text" name="valor_minimo" value="'.htmlentities(Tools::getValue('valor_minimo', $this->ifmb_valorminimo), ENT_COMPAT, 'UTF-8').'" style="width: 300px;" /></td></tr><tr><td colspan="2" align="center"><input class="button" name="btnSubmit" value="Guardar Dados" type="submit" /></td></tr>
				</table>
			</fieldset>
		</form>
		<form action="'.Tools::htmlentitiesUTF8($_SERVER['REQUEST_URI']).'" method="post">
	...
	private function _displayCallbackForm()
	{
		$this->_html .='<br /><br />
			<fieldset>
			<legend><img src="../modules/multibancoprepagamento/logo.gif" />Dados Callback</legend>
	...
	public function getContent()
	{
		...
		if(Configuration::get('MULTIBANCO_ENTIDADE')!="" && Configuration::get('MULTIBANCO_SUBENTIDADE')!="" && Configuration::get('MULTIBANCO_PERCENTAGEM')!="" || Configuration::get('MULTIBANCO_VALOR_MINIMO')!="")
	...
	public function hookPaymentReturn($params)
	{
				...
				$state < 14)
		{
			...
			$tot = $this->getPrePaidValue($params['total_to_pay']);
			
			$entidade = $this->ifmb_entidade;
			$referencia = $this->GenerateMbRef($this->ifmb_entidade,$this->ifmb_subentidade,$params['objOrder']->id,$tot);
			$total = Tools::displayPrice($tot, $params['currencyObj'], false);
	...
	public function getMultibancoOrderDb($entidade,$referencia,$order = 0, $count=false)
	{
		$select="order_id";
	...
	function getPrePaidValue($order_value) {
		$valorMinimo = $this->ifmb_valorminimo;
		$percentagem = $this->ifmb_percentagem;
	
		if ($valorMinimo < $order_value) {
			$percent = 0;
			...
			$tot = (($order_value * $percent) > $valorMinimo) ? ($order_value * $percent) : $valorMinimo;
		} else {
			$tot = $order_value;
		}
	...
	public function callback($chave, $entidade, $referencia, $valor)
	{
		...
		if($chave == $chaveReg){
			$orderId = $this->getMultibancoOrderDb($entidade, $referencia);
